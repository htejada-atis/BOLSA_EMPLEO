...
public class TestBEPControladorGestionEvaluadores {
	...
	private VistaEvaluadores getVistaConAreas() throws ServletException, IOException {
		...
		peticion.setParameter(ControladorGestionEvaluadores.PARAM_DEPARTAMENTO,
				bean.getDepartamentos().get(1).getCodNum().toString());
	...
	public void testA04AgregarEvaluador() throws SQLException, ServletException, IOException {
		...
		peticion.setParameter(ControladorGestionEvaluadores.PARAM_AREAS,
				"[" + bean2.getDatatableAreas().getData().get(0).getCodNum().toString() + "]");
		peticion.setParameter(ControladorGestionEvaluadores.PARAM_DEPARTAMENTO,
				bean.getDepartamentos().get(1).getCodNum().toString());
	...
	public void testE01ObtenerEvaluadoresParametroNoValido() throws SQLException, ServletException, IOException {
		...
		PeticionHttp peticion = UtilsTestBolsaEmpleo.peticionAutenticadaPersonal();
		peticion.setParameter(ControladorGestionEvaluadores.PARAM_ACCION,
				ControladorGestionEvaluadores.ACCION_DATATABLE_EVALUADORES);
		peticion.setParameter(ControladorGestionEvaluadores.PARAM_AREA,
				bean.getDatatableAreas().getData().get(0).getCodNum().toString());
	 ...
	 */
	@Test
	public void testE03AgregarEvaluadorRolNoValido() throws ServletException, IOException {
		PeticionHttp peticion = UtilsTestBolsaEmpleo.peticionAutenticadaPersonal();
	 ...
	 */
	@Test
	public void testE04AgregarEvaluadorNoValido() throws ServletException, IOException {
		PeticionHttp peticion = UtilsTestBolsaEmpleo.peticionAutenticadaPersonal();
		peticion.setParameter(ControladorGestionEvaluadores.PARAM_ACCION, ControladorGestionEvaluadores.ACCION_AGREGAR_EVALUADORES);
		...
		assertEquals(MENSAJE_CON_ERROR, 1, bean3.getMensajesDeError().size());
		assertEquals(MENSAJE_SIN_EXITO, 0, bean3.getMensajesDeExito().size());
		assertEquals(MENSAJE_CON_ERROR_ESPERADO, ModeloUsuarioBolsaEmpleo.MENSAJE_ERROR_USUARIO_CON_DOCUMENTO_NO_EXISTE, bean3.getMensajesDeError().get(0));
	}
	 ...
	 */
	@Test
	public void testE05AgregarEvaluadorYaExistente() throws ServletException, IOException {
		PeticionHttp peticion = UtilsTestBolsaEmpleo.peticionAutenticadaPersonal();
		peticion.setParameter(ControladorGestionEvaluadores.PARAM_ACCION, ControladorGestionEvaluadores.ACCION_AGREGAR_EVALUADORES);
		peticion.setParameter(ControladorGestionEvaluadores.PARAM_NOMBRE_EVALUADOR, "comision2");
