	...
	<form id="plaza_ofertada" class="be-form" method="post" action="<%=request.getRequestURI()%>">
		<div class="form-group-container col2">
			<div class="form-group">
				...
				<input class="form-input-custom" id="plaza_area" type="text" value="<%= EscapaHTML.escapa(plaza.getArea().getIdAreaExterno() + " " + plaza.getArea().getDescripcion()) %>"
						readonly disabled/>
			</div>
			<div class="form-group">
				<label for="plaza_centro_destino" class="bold-label">Centro destino:</label>
				<input class="form-input-custom" id="plaza_centro_destino" type="text" value="<%= EscapaHTML.escapa(plaza.getCentroDestino()) %>"
						readonly disabled/>
			</div>
		</div>
		<div class="form-group-container col2">
			<div class="form-group">
				...
				<input class="form-input-custom" id="plaza_cuatrimestre" type="text" value="<%= EscapaHTML.escapa(plaza.getCuatrimestre()) %>"
						readonly disabled/>
			</div>
		</div>
		<div class="form-group-container col2">
			<div class="form-group">
				...
				<input class="form-input-custom" id="plaza_dedicacion" type="text" value="<%= EscapaHTML.escapa(plaza.getDedicacion().getTexto()) %>"
						readonly disabled/>
			</div>
			<div class="form-group">
				<label for="plaza_dedicacion" class="bold-label">Sueldo:</label>
				<input class="form-input-custom" id="plaza_dedicacion" type="text" value="<%= EscapaHTML.escapa(plaza.getDedicacion().getSueldo().toString()) %>"
						readonly disabled/>
			</div>
		</div>
		<div class="form-group-container col1">
			<div class="form-group">
				...
				<textarea class="form-input-custom" id="plaza_justificacion" rows="3"
						readonly disabled><%= EscapaHTML.escapa(plaza.getJustificacion()) %></textarea>
			</div>
		</div>
		...
		<div class="form-group-container col2">
			<div class="form-group">
				...
				<input class="form-input-custom" id="plaza_duracion_prevista" type="text" value="<%= EscapaHTML.escapa(plaza.getDuracionPrevista()) %>"
						readonly disabled/>
			</div>
			<div class="form-group">
				<label for="plaza_fecha_fin_oferta" class="bold-label">Fecha fin oferta:</label>
				<input class="form-input-custom" id="plaza_fecha_fin_oferta" type="text" value="<%= plaza.getFechaFinOferta() != null ? Formateador.formatoFecha(plaza.getFechaFinOferta(), Formateador.FORMATO_FECHA_DDMMYYYY_HHMMSS) : "" %>"
						readonly disabled/>
			</div>
		</div>
...
<%	if (plaza.getHorario() != null) { %>
		document.getElementById("plaza_descargar_horario").addEventListener("click", function() {
			window.open("<%=ControladorDescargaFicheros.URL_DESCARGA_FICHEROS%>"
					+ "<%="?a=" + ControladorDescargaFicheros.ACCION_DESCARGAR_HORARIO_CANDIDATO + "&" + ControladorDescargaFicheros.PARAM_PLAZA_OFERTADA + "=" + plaza.getCodNum()%>");
		});
<%	} %>
		...
		Atis.confirmDialog("Aceptar plaza", "Al aceptar confirma que está interesado y entrará en el proceso de selección para la plaza.", {
				'Si': function(row) {
					var params = {
							...
							'<%= ControladorPlazasOfertadas.PARAM_PLAZA_OFERTADA %>': <%= plaza.getCodNum() %>
					};
