...
<div class="bolsa-empleo">	
	...
	<h2>Paso 2: Asignaci�n de m�ritos a �reas</h2>
	<h3><%= bean.getSolicitud().getConvocatoria().getDescripcion() %></h3>
	
	<p>Para cada �rea seleccione hasta un m�ximo de [<%= bean.getSolicitud().getConvocatoria().getNumMeritosPorBloque() %>] m�ritos por bloque</p>
	
	<table class="bluetable bolsaempleo" id="tableAreas">
		<tr>
			<th scope="col"	style="width:15%">C�digo</th>
			<th scope="col"	style="width:65%">Nombre</th>
			<th scope="col"	style="width:20%">N� m�ritos en �rea</th>
		</tr>
		<tbody>		
		</tbody>
		<tfoot>
	...
	<table class="bluetable bolsaempleo" id="tableMeritos" style="visibility: collapse">
		<tr>
			...
			<th scope="col"	style="width:10%">C�digo �tem</th>
			<th scope="col"	style="width:40%">M�rito</th>
...
<script>
	...
	function renderIndividualizado(row) {
		...
		$('button', texto).on('click', function() {
			...
			var message = 
				'<h3>Selecciona la afinidad para el m�rito</h3><br/>' +
				'<p>M�rito: ' + Atis.escapeHtml(row.merito.item.nombre) + '</p>' +
				(row.merito.descripcion ? "<p>Descripci�n: " + Atis.escapeHtml(row.merito.descripcion) + "</p>" : "") +
				...
				'<p>Tipo de m�rito: <strong>individualizado</strong></p><br/>' +
				'<p>Selecciona el grado de afinidad del m�rito con el area:</p>' +
				'<p><select><option value="0">Sin afinidad (0 %)</option>' + options + '</select></p>'
			;
			
			Atis.alertDialog("Seleccionar afinidad m�rito individualizado", $(message), function(dialog) {
	...
	function renderNoIndividualizado(row) {
		...
		$('button', texto).on('click', function() {
			var message = 
				...
				'<p>Valor: ' + row.merito.valor + '</p>' +
				'<br/><p>Area: <strong><%= bean.getArea() != null ? bean.getArea().getArea().getDescripcion() : "" %></strong></p>' +
				'<p>Tipo de m�rito: <strong>no individualizado</strong></p><br/>' +
				'<p>Introduce el valor para cada grado de afinidad con el �rea (la suma de todas debe ' + row.merito.valor + ')</p>'
			;
			...
			var actualizaTotal = function() {
				...
				if (total > row.merito.valor) {
					$('p.mensaje').text('El total es superior al valor del m�rito. M�ximo ' + row.merito.valor);
					$('p.mensaje').show();
				} else {
					$('p.mensaje').hide();
				}
			...
			Atis.alertDialog("Seleccionar afinidad m�rito no individualizado", node, function(dialog) {
				var option = $('select option:selected', dialog);
				...
				if (total != row.merito.valor) {
					$('p.mensaje').text('El total debe ser igual al valor del m�rito. Valor de m�rito ' + row.merito.valor);
					$('p.mensaje').show();
					return false;
				}
	...
	$(document).ready(function() {
		...
		var tableAreas = new Atis.DataTable('#tableAreas', {
		    ...
		    "title": 'Mis �reas para esta convocatoria',
		...
		<% if (bean.getArea() != null) { %>
			...
			var tableMeritos = new Atis.DataTable('#tableMeritos', {
			    ...
			    "selectable": true,
			    "filterable": true,
			    "title": 'Mis m�ritos: <%=bean.getArea().getArea().getDescripcion()%>',
			    ...
			    "columns": [
		        	...
		        	{'data': 'merito.item.nombre', 'filter': true, 'render': function(row) {
			        	return row.merito.item.nombre + "<br/> Descripci�n del usuario: " + row.merito.descripcion;
		        	}},
		        	...
		        	{'data': 'codNum', 'filter': false, 'render': function(row) {
		        		...
		        		if (row.merito.item.afinidad) {
		        			...
		        			return '<span title="El m�rito no tiene afinidad con el �rea seleccionada"></span>';
		        		}		        				        	
		        	}},
		        	{'data': 'excluido', 'order': {'active': false}, 'filter': {'type': 'selectBoolean', 'true': 'Excluido', 'false': 'No excluido', 'optionDefault': 'false'}, 'render': function(row) {
		        		if (row.excluido) {
		        			return "<div title='M�rito excluido' class='circle-false'></div>";
		        		} else {
		        			return "<div title='M�rito no excluido' class='circle-true'></div>";
		        		}
		        	}},
		        ],
			});
