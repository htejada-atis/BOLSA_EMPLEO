...
String texto = BolsaEmpleoUtils.getParamForm(request, ControladorNoticias.PARAM_TEXTO, noticia != null ? noticia.getTexto() : "");
String enlace = BolsaEmpleoUtils.getParamForm(request, ControladorNoticias.PARAM_ENLACE, noticia != null && noticia.getEnlace() != null ? noticia.getEnlace() : "");
String fecha = BolsaEmpleoUtils.getParamForm(request, ControladorNoticias.PARAM_FECHA, noticia != null ? Formateador.formatoFecha(noticia.getFecha(), Formateador.FORMATO_FECHA_DDMMYYYY) : "");
...
<div class="bolsa-empleo">
    ...
    <form id="actualizar_noticia" class="be-form" method="post" action="<%=request.getRequestURI()%>">
    	<input type="hidden" name="<%=ControladorNoticias.PARAM_ACCION%>" id="accion_formulario" 
    			value="<%=noticia != null ? ControladorNoticias.ACCION_EDITAR_NOTICIA : ControladorNoticias.ACCION_AGREGAR_NOTICIA%>" />
		<input type="hidden" name="<%=ControladorNoticias.PARAM_ID%>" id="noticia_id" 
				value="<%=noticia != null ? noticia.getCodNum() : ""%>" />
		<div class="form-group-container col1">
			<div class="form-group">
	    		<label for="noticia_texto" class="bold-label">Texto:</label>
	    		<input class="form-input-custom" type="text" name="<%=ControladorNoticias.PARAM_TEXTO%>" id="noticia_texto" value="<%=texto%>" required/>
	    	</div>
		</div>
    	<div class="form-group-container col2">
    		<div class="form-group">
	    		<label for="noticia_enlace">Enlace:</label>
	    		<input class="form-input-custom" type="text" name="<%=ControladorNoticias.PARAM_ENLACE%>" id="noticia_enlace" value="<%=enlace%>"/>
	    	</div>
	    	<div class="form-group">
	    		<label for="noticia_fecha"><strong>Fecha</strong>:</label>
	    		<input class="form-input-custom" type="text" name="<%=ControladorNoticias.PARAM_FECHA%>" id="noticia_fecha" autocomplete="off" 
	    		value="<%=fecha%>" required/>
	    	</div>
    	</div>
    	<div class="form-check">
    		<label for="noticia_publica" class="bold-label"><input type="checkbox" id="noticia_publica" name="<%=ControladorNoticias.PARAM_PUBLICA%>" 
    		value="<%=publica%>" <%=(publica ? "checked=''" : "")%> />Pública</label>
    	</div>
    	<div class="form-btn">
    		<input id="noticia_enviar" type="submit" name="<%=ControladorNoticias.PARAM_ENVIAR%>" value="<%=noticia != null ? "Guardar cambios" : "Insertar noticia"%>"/>
    	</div>
    </form>
...
<script>
	...
	function enviarNoticia(event, submit_input) {
		...
		<%if(noticia != null) {%>
			...
			input_accion.value = '<%=ControladorNoticias.ACCION_EDITAR_NOTICIA%>';
			input_id = document.getElementById("noticia_id");
			input_id.value = '<%=bean.getNoticia().getCodNum()%>';
		<%} else {%>
			input_accion = document.getElementById("accion_formulario");
			input_accion.value = '<%=ControladorNoticias.ACCION_AGREGAR_NOTICIA%>';
		<% } %>
