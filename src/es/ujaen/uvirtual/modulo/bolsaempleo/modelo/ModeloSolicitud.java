...
public class ModeloSolicitud {		
	...
	public static final String BEPSBO_CODNUM = "BEPSBO_CODNUM";
	...
	/**
	 * Devuelve un merito solicitud asociado a una solicitud y una bolsa de una convocatoria anterior .
	 ...
	 * @throws UVException .
	 */
	public MeritoSolicitud getMeritoSolicitudByConvocatoriaAnterior(Convocatoria convocatoriaActual, Bolsa bolsa, Merito merito) throws SQLException, UVException {
		String consulta = "SELECT bepsbm.*"
				+ " FROM TBEP_SOL_BOL_MERITOS bepsbm"
				+ " INNER JOIN TBEP_SOLICITUD_BOLSAS bepsbo ON bepsbo.CODNUM = bepsbm.BEPSBO_CODNUM"
				...
				+ "     AND bepsol.BEPCON_CODNUM != ?"
				+ "     AND bepsbo.BEPBOL_CODNUM = ?"
				+ "     AND (bepsbm.FLGVALIDADO != 'N' OR bepsbm.FLGEXCLUIDO != 'N')"
				+ " ORDER BY bepsol.BEPCON_CODNUM DESC FETCH FIRST 1 ROW ONLY";
		...
		try (Connection conexion = ConexionUvirtual.obtenerInstancia();
			...
			int params = 1;
			stmt.setInt(params++, merito.getCodNum());
			stmt.setInt(params++, convocatoriaActual.getCodNum());
			stmt.setInt(params++, bolsa.getCodNum());
	...
	/** El usuario selecciona un mérito de una solicitud anterior para una bolsa en la solicitud actual .
	 * @param solicitud .
	 * @param bolsa .
	 ...
	 * @throws SQLException .
	 */
	public void asignarMeritoAnteriorASolicitudBolsa(Solicitud solicitud, Bolsa bolsa, MeritoSolicitud meritoSolicitud, UsuarioBolsaEmpleo usuarioUpdate)
		...
		try (Connection conexion = ConexionUvirtual.obtenerInstancia()) {
			...
			try {
				...
				try (PreparedStatement stmt = conexion.prepareStatement(consulta, new String[] {CODNUM})) {
					...
					codNumSBM = rs.getInt(1);
				}
				
				boolean individualizado = meritoSolicitud.getItem() != null 
						? meritoSolicitud.getItem().getIndividualizado() : meritoSolicitud.getMerito().getItemBaremacion().getIndividualizado();
				boolean afinidad = meritoSolicitud.getItem() != null ? meritoSolicitud.getItem().getAfinidad() != null
						: meritoSolicitud.getMerito().getItemBaremacion().getAfinidad() != null;
				
				if (individualizado && afinidad) {
					...
					try (PreparedStatement stmt = conexion.prepareStatement(sqlInsert)) {
						int indexParam = 1;
						stmt.setInt(indexParam++, codNumSBM);
						stmt.setInt(indexParam++, meritoSolicitud.getValoraciones().get(0).getAfinidad().getCodNum());
						stmt.setString(indexParam++, usuarioUpdate.getCodCuenta());
						stmt.executeUpdate();
					}
				} else if (afinidad) {
					for (MeritoSolicitudValoracion valoracion: meritoSolicitud.getValoraciones()) {
						String sqlInsert = "INSERT INTO TBEP_SOL_BOL_MER_VALORACION (BEPSBM_CODNUM, BEPAFI_CODNUM, VALOR, UID_USUARIO) VALUES (?,?,?,?)";
						
						try (PreparedStatement stmt = conexion.prepareStatement(sqlInsert)) {
							int indexParam = 1;
							stmt.setInt(indexParam++, codNumSBM);
							stmt.setInt(indexParam++, valoracion.getAfinidad().getCodNum());
							stmt.setDouble(indexParam++, valoracion.getValor());
							stmt.setString(indexParam++, usuarioUpdate.getCodCuenta());
							stmt.executeUpdate();
						}
					}
	...
	/** Devuleve el id de la solicitud bolsa en base al id de una solicitud y el id de una bolsa .
	 * @param solicitud .
	 * @param bolsa .
	 * @param conexion .
	 * @return .
	 ...
	 * @throws SQLException  .
	 */
	private Integer getIdSolicitudBolsa(Solicitud solicitud, Bolsa bolsa, Connection conexion) throws SQLException, UVException {
		...
		try (PreparedStatement stmt = conexion.prepareStatement(consulta)) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				if (!rs.next()) {
					throw new UVException("No existe la solicitud bolsa");
				}
