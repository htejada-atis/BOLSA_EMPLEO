...
public class ModeloEstadoCandidato {
	 ...
	 */
	public CandidatoEstado getEstadoCandidatoByUsuarioBolsa(UsuarioBolsaEmpleo candidato, Bolsa bolsa)
	 ...
	 * @throws UVException  error si no existe la area .
	 */
	public BolsaEmpleoDataTable<CandidatoEstado> listaEstadosCandidato(Map<String, String[]> params,
			UsuarioBolsaEmpleo candidato) throws SQLException, UVException {
		...
		String consulta = "SELECT bepbol.CODNUM AS BEPBOL_CODNUM, bepplo.CODNUM AS BEPPLO_CODNUM, bepesc.CODNUM,"
				+ "     bepesc.ESTADO, bepesc.BEPUSU_CODNUM" + " FROM TBEP_BOLSAS bepbol "
				+ " INNER JOIN TBEP_AREAS bepare ON bepare.CODNUM = bepbol.BEPARE_CODNUM"
				+ " LEFT JOIN TBEP_ESTADO_CANDIDATOS bepesc ON bepesc.BEPBOL_CODNUM = bepbol.CODNUM AND bepesc.BEPUSU_CODNUM = ?"
				+ " LEFT JOIN TBEP_PLAZAS_OFERTADAS bepplo ON bepplo.CODNUM = bepesc.BEPPLO_CODNUM" + " WHERE 1=1";
	 ...
	 * @throws UVException  error si no existe la area .
	 */
	public BolsaEmpleoDataTable<CandidatoEstado> listaEstadosCandidatoDisponiblesPlaza(Map<String, String[]> params,
			PlazaOfertada plaza) throws SQLException, UVException {
		List<CandidatoEstado> estados = new ArrayList<>();
		BolsaEmpleoDataTable<CandidatoEstado> dataTable = new BolsaEmpleoDataTable<>(params);
	...
	/**
	 * Genera un listado con el estado de contratación de los cantidados.
	 * @param separator .
	 * @return .
	 * @throws SQLException .
	 */
	public List<String[]> listadoEstadoCandidatosCsv(String separator) throws SQLException {
		String consulta = ""
			+ " SELECT "
			+ "		bepusu.VUAJA_IDNIF, "
			+ "		bepusu.VUAJA_STRNOMBRE, "
			+ "		bepusu.VUAJA_STRAPELLIDO1, "
			+ "		bepusu.VUAJA_STRAPELLIDO2, "
			+ "		bepesc.ESTADO, "
			+ "		bepare.ID_AREA_CONOCIMIENTO, "
			+ "		bepare.DES_AREA_CONOCIMIENTO, "
			+ "		MAX(bepeschto.FECHALOG) AS FECHALOG "
			+ " FROM TBEP_USUARIOS bepusu "
			+ " LEFT JOIN TBEP_ESTADO_CANDIDATOS bepesc ON bepesc.BEPUSU_CODNUM = bepusu.CODNUM "
			+ " LEFT JOIN TBEP_BOLSAS bepbol ON bepbol.CODNUM = bepesc.BEPBOL_CODNUM "
			+ " LEFT JOIN TBEP_AREAS bepare ON bepare.CODNUM = bepbol.BEPARE_CODNUM "
			+ " LEFT JOIN TBEP_HTO_ESTADO_CANDIDATOS bepeschto ON ( "
			+ "		bepeschto.CODNUM = bepesc.CODNUM "
			+ "		AND bepeschto.ESTADO = bepesc.ESTADO "
			+ "		AND bepeschto.LOG IN ('AFTER_INSERT', 'AFTER_UPDATE')) "
			+ " WHERE bepusu.ROL = ? AND bepusu.FLGBORRADO = 'N' "
			+ " GROUP BY bepusu.VUAJA_IDNIF, bepusu.VUAJA_STRNOMBRE, bepusu.VUAJA_STRAPELLIDO1, bepusu.VUAJA_STRAPELLIDO2, "
			+ "			 bepesc.ESTADO, bepare.ID_AREA_CONOCIMIENTO, bepare.DES_AREA_CONOCIMIENTO ";
		...
		try (Connection conexion = ConexionUvirtual.obtenerInstancia(); PreparedStatement stmt = conexion.prepareStatement(consulta)) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				rows.add(new String[] {"DNI", "NOMBRE Y APELLIDOS", "ESTADO CONTRATACION", "FECHA ESTADO", "COD. AREA", "AREA"});
			
				while (rs.next()) {
					String dni = BolsaEmpleoUtils.string2csv(rs.getString("VUAJA_IDNIF"), separator);
					String nombre = BolsaEmpleoUtils.string2csv(
							rs.getString("VUAJA_STRNOMBRE") + ", "
							+ rs.getString("VUAJA_STRAPELLIDO1") + " "
							+ rs.getString("VUAJA_STRAPELLIDO2"), separator);
					String estado = rs.getString("ESTADO") == null ? "DISPONIBLE" : rs.getString("ESTADO");
					String fecha = BolsaEmpleoUtils.date2csv((Date) rs.getTimestamp("FECHALOG"), separator);
					String codArea = BolsaEmpleoUtils.string2csv(rs.getString("ID_AREA_CONOCIMIENTO"), separator);
					String desArea = BolsaEmpleoUtils.string2csv(rs.getString("DES_AREA_CONOCIMIENTO"), separator);
															
					rows.add(new String[] {dni, nombre, estado, fecha, codArea, desArea});
				}
			}
		}
	 ...
	 * @throws UVException  .
	 */
	public List<CandidatoEstado> listaEstadosCandidatoPlazaDisponibles(PlazaOfertada plaza)
			throws SQLException, UVException {
		List<CandidatoEstado> estados = new ArrayList<>();
	 ...
	 * @throws UVException  error si no existe la area .
	 */
	public void cambiarEstadosCandidato(CandidatoEstado candidato, String estado, List<Bolsa> bolsas,
	 ...
	 * @throws UVException  error si no existe la area .
	 */
	public void cambiarEstadoCandidato(CandidatoEstado candidato, String estado, Bolsa bolsa,
			UsuarioBolsaEmpleo usuarioUpdate) throws SQLException, UVException {
		try (Connection conexion = ConexionUvirtual.obtenerInstancia()) {
			...
			try {
				...
				if (candidatoEstado == null) {
					...
					if (estado.equals(ESTADO_DISPONIBLE) || estado.equals(ESTADO_NO_DISPONIBLE)
							|| estado.equals(ESTADO_SUSPENSION_PROVISIONAL)) {
	...
	public void insertaEstadoCandidato(CandidatoEstado candidatoEstado, UsuarioBolsaEmpleo usuarioUpdate,
			Connection conexion) throws SQLException, UVException {
		if (candidatoEstado == null) {
		...
		try (PreparedStatement stmt = conexion.prepareStatement(consulta)) {
			...
			stmt.setString(parameterIndex++,
					candidatoEstado.getEstado() != null ? candidatoEstado.getEstado() : ESTADO_DISPONIBLE);
	...
	public void actualizaEstadoCandidato(CandidatoEstado candidatoEstado, UsuarioBolsaEmpleo usuarioUpdate,
			Connection conexion) throws SQLException, UVException {
		if (candidatoEstado == null) {
			throw new UVException(String.format(MENSAJE_ERROR_OBJETO_VACIO, "insertar"));
		}
	 ...
	 * @throws SQLException .
	 */
	public CandidatoEstado createCandidatoEstadoFromResultSet(ResultSet rs, UsuarioBolsaEmpleo usuario,
			boolean extraParams) throws SQLException, UVException {
		CandidatoEstado candidato = new CandidatoEstado(
				usuario == null ? ModeloUsuarioBolsaEmpleo.obtenerInstancia().getUsuarioById(rs.getInt(BEPUSU_CODNUM))
						: usuario);
		...
		candidato.setPlaza(rs.getInt(BEPPLO_CODNUM) != 0
				? ModeloPlazaOfertada.obtenerInstancia().getPlazaOfertadaById(rs.getInt(BEPPLO_CODNUM))
				: null);
